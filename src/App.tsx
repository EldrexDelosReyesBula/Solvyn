...
function AutoComputeDemo() {
  const { history, clearHistory, exportHistory, importHistory } = useSolvyn();
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = 'solvyn-history.json';
    a.click();
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={handleImport} className="text-xs px-2 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">Import</button>
                  <button onClick={handleExport} className="text-xs px-2 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">Export</button>
                  <button onClick={clearHistory} className="text-xs px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors">Clear</button>
                </div>
              </div>
              <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                {[...history].reverse().map((item) => (
                  <motion.div 
                    ...
                    key={item.id} 
                    className="text-sm flex justify-between items-center bg-zinc-50 dark:bg-zinc-950 px-3 py-2 rounded-md border border-zinc-100 dark:border-zinc-800"
                  >
...
- **History & Storage**: Tracks input and result history with configurable storage adapters (Memory, LocalStorage, IndexedDB, Cloud). Includes import/export and max items limit.
...
- **Physics-Driven Animations**: Built-in \`motion\` support for smooth input expansion, tooltips, and evaluation states.
...
function MyComponent() {
  const { solve, result, history, clearHistory, exportHistory, importHistory, events } = useSolvyn({ 
    autoCompute: true,
    historyEnabled: true,
...
const config = {
  ...
  maxHistoryItems: 100 // Limit the number of stored items
};
\`\`\`
...
Extensible solvers or computation modules. You can register custom plugins that match specific input patterns and provide their own solve logic.
